...
-- t が拡張子のキー
-- val がicon, color, name を格納したテーブル
-- k がキー
-- v がバリュー

local function isnew_palette(table, val)
  local is_new = true
  for _, v in pairs(table) do
    if v.color == val.color then
      is_new = false
      return is_new
    end
  end
...
local function create_palette(val)
  return {
    color = val.color,
    cterm_color = val.cterm_color,
    icons = { val.icon },
  }
end

local function search_palette_index(table, color)
  local index = 0
  ...
  return error('can not find index.\n this function is only used for exist keys')
end

local function isnew_icon(table, icon)
  local is_new = true
  for _, v in pairs(table) do
    if v == icon then
      is_new = false
...
local function append_palette(palettes, val)
  if isnew_palette(palettes, val) then
    palettes[#palettes + 1] = create_palette(val)
  else
    local index = search_palette_index(palettes, val.color)
    local palette = palettes[index]
    -- if isnew_icon(palette['icons'], val.icon) then
    table.insert(palette['icons'], val.icon)
    -- end
  end
end

local function table_icons(table)
  local tmp = {}
...
-- needed for print debugging of arrays
local function strings_icons(table)
  local tmp = ''
  for _, val in pairs(table) do
    tmp = tmp .. "'" .. val .. "', "
  end
...
-- table for store colors
local palettes = {}
local colorcodes = {}
local GlyphNDeviconPalettes = {}
...
-- cterm_color: number
-- icons: string
for _, val in pairs(devicons) do
  append_palette(palettes, val)
end

-- create palette
for k, val in pairs(palettes) do
  if val.cterm_color then
    colorcodes[#colorcodes + 1] = { ctermfg = val.cterm_color, guifg = val.color }
  else
    colorcodes[#colorcodes + 1] = { ctermfg = k, guifg = val.color }
  end
  -- vimscript array indexes start at 0,
  -- but lua array indexes start at 1,
  -- so adjustments are needed
  GlyphNDeviconPalettes['GlyphCustomPalette' .. k - 1] = table_icons(val.icons)
end
vim.api.nvim_command('highlight default GlyphCustomPaletteDirectory guifg = #73bae4')
GlyphNDeviconPalettes['GlyphCustomPaletteDirectory'] = { '', '', '', '', '', '' }

vim.g['glyph_palette_custom_colors'] = colorcodes
vim.g['glyph_palette#palette'] = GlyphNDeviconPalettes

local FernRootText = 'highlight link FernRootText SignColumn'
vim.api.nvim_command([[highlight link FernRootText SignColumn]])
vim.api.nvim_command([[highlight link FernBranchText SignColumn]])
...
-- local fernname, fernlight = require('nvim-web-devicons').get_icon('test.dto.ts')
-- print(fernname, fernlight)
